...
export default function ManageOnlineTests() {
  ...
  const handleUpdateExam = (examId) => {
    window.dispatchEvent(new Event("navigation-start"));
    router.push(`/create-test/online?editId=${examId}`);
  };
  ...
  const handleDelete = async (examId) => {
    try {
      ...
      showNotification("Online test deleted successfully!", "success");
      await loadExams();
    } catch (err) {
      console.error("Error deleting exam:", err);
      showNotification("Failed to delete exam: " + err.message, "error");
      throw err;
    } finally {
      setLoading(false);
    }
          ...
          <div className="p-4 border-b border-gray-100 bg-white flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <Select value={sortBy} onValueChange={setSortBy}>
                <SelectTrigger className="w-auto bg-white border-gray-200 text-gray-700 font-medium hover:bg-gray-50 focus:ring-2 focus:ring-blue-500/20">
                    ...
                    {currentExams.map((exam, index) => (
                      ...
                      >
                        ...
                        <TableCell className="text-center">
                          <Badge
                            variant="outline"
                            className={getStatusInfo(exam).className}
                          >
              ...
              <>
                ...
                <div className="px-6 py-3 border-t border-gray-100/0 bg-transparent mt-2">
                  <p className="text-gray-500 text-xs text-center">
                    Page {currentPage} of {totalPages} · {filteredExams.length}{" "}
                    test{filteredExams.length !== 1 ? "s" : ""} total
                  </p>
                </div>
              </>
            )}
